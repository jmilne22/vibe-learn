...
function buildCourse(slug) {
    ...
    function buildExerciseScripts(moduleId) {
        ...
        let scripts = `    <script src="exercise-core.js"></script>\n    <script src="course.js"></script>\n    <script src="exercise-renderer.js"></script>\n    <script src="module-loader.js"></script>`;
        if (fs.existsSync(path.join(CONTENT_DIR, 'syntax-reference.yaml'))) {
            scripts += `\n    <script src="data/syntax-reference.js"></script>`;
        }
        return scripts;
    }
    ...
    // 11b. Compile syntax-reference.yaml → data/syntax-reference.js (if exists)
    const syntaxRefPath = path.join(CONTENT_DIR, 'syntax-reference.yaml');
    if (fs.existsSync(syntaxRefPath)) {
        console.log('Compiling syntax reference...');
        const syntaxRaw = fs.readFileSync(syntaxRefPath, 'utf8');
        let syntaxParsed;
        try {
            syntaxParsed = yaml.load(syntaxRaw) || {};
        } catch (e) {
            console.error(`  Invalid YAML in syntax-reference.yaml:`, e.message);
            process.exit(1);
        }
        fs.writeFileSync(
            path.join(COURSE_DIST, 'data', 'syntax-reference.js'),
            `// Auto-generated from syntax-reference.yaml - do not edit directly\nwindow.syntaxReference = ${JSON.stringify(syntaxParsed)};\n`
        );
        const refCount = Object.keys(syntaxParsed).length;
        console.log(`  data/syntax-reference.js (${refCount} concepts)`);
    }

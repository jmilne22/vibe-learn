 ...
 */
// Per-course collector so the session runner can serve predict blocks as
...
function processPredictBlocks(md, pageId) {
    ...
    return md.replace(/<predict(\s+[^>]*)?>([\s\S]*?)<\/predict>/g, (match, attrs, inner) => {
        ...
        const pm2 = pageId.match(/module(\d+)(?:-(\d+))?$/);
        predictIndexCollector.push({
            id,
            prompt,
            codeHtml,
            outputHtml,
            moduleId: pm2 ? parseInt(pm2[1], 10) : null,
            sectionNum: pm2 && pm2[2] ? parseInt(pm2[2], 10) : null
        });
...
// ---------------------------------------------------------------------------
/**
 * Converts GitHub-style callout syntax rendered by marked:
 *
 *   > [!tip] Title Here
 *   > Content paragraph
 *
 * `marked` first renders this as a <blockquote> with the [!type] text inside.
 * This post-processor finds those blockquotes and transforms them into:
 *
 *   <div class="callout tip">
 *     <div class="callout-header"><span class="callout-type">Tip</span> Title Here</div>
 *     <div class="callout-body"><p>Content paragraph</p></div>
 *   </div>
 *
 * Regular blockquotes (without [!type]) are left unchanged.
 */
function processCallouts(html) {
    const calloutTypes = 'tip|warning|info|definition|example';
    const pattern = new RegExp(
        '<blockquote>\\s*<p>\\[!(' + calloutTypes + ')\\]([\\s\\S]*?)</p>([\\s\\S]*?)</blockquote>',
        'gi'
    );

    return html.replace(pattern, function(_match, type, firstBlock, rest) {
        const typeLower = type.toLowerCase();
        const typeLabel = typeLower.charAt(0).toUpperCase() + typeLower.slice(1);

        // firstBlock may contain " Title\nBody text" or just "\nBody text" or " Title"
        // Title is only the text on the same line as [!type] (before first newline)
        const newlineIdx = firstBlock.indexOf('\n');
        const title = newlineIdx >= 0 ? firstBlock.substring(0, newlineIdx).trim() : firstBlock.trim();
        const firstBodyLines = newlineIdx >= 0 ? firstBlock.substring(newlineIdx + 1).trim() : '';

        const headerTitle = title ? ' ' + title : '';
        const header = `<div class="callout-header"><span class="callout-type">${typeLabel}</span>${headerTitle}</div>`;

        // Collect body: remaining lines from first <p> + any subsequent elements
        let body = '';
        if (firstBodyLines) {
            body += '<p>' + firstBodyLines + '</p>';
        }
        if (rest.trim()) {
            body += rest.trim();
        }

        return `<div class="callout ${typeLower}">${header}<div class="callout-body">${body}</div></div>`;
    });
}
...
function buildCourse(slug) {
    ...
    // sidebarPages are populated dynamically inside the module generation loop (step 7)
    // to support both single-page modules and split modules (directories).
    // Plugin sidebar entries are added after all modules are processed.
    ...
    function extractSectionTitle(html) {
        ...
        if (!match) return { title: 'Untitled', html: html };
        const title = match[1].replace(/<[^>]+>/g, '').trim();
        const body = html.replace(match[0], '');
        return { title, html: body };
    }

    // --- Pass 1: Populate sidebarPages for ALL modules (needed for nav buttons) ---
    const splitModuleSections = {}; // mod.id -> [{ file, title, mdPath }]
    modules.forEach(mod => {
        const mdFile = path.join(LESSONS_DIR, mod.file + '.md');
        const mdDir = path.join(LESSONS_DIR, mod.file);

        if (fs.existsSync(mdDir) && fs.statSync(mdDir).isDirectory()) {
            mod.isSplit = true;
            const sectionFiles = fs.readdirSync(mdDir).filter(f => f.endsWith('.md')).sort();
            const sections = [];
            ...
            if (mod.hasExercises) {
                const exFile = `module${mod.id}-exercises.html`;
                sidebarPages.push({
                    file: exFile,
                    label: `${mod.id} Exercises`,
                    num: `${mod.id}.E`,
                    title: 'Exercises',
                    type: 'exercises',
                    moduleId: mod.id,
            ...
            splitModuleSections[mod.id] = sections;
        } else if (fs.existsSync(mdFile)) {
            sidebarPages.push({
                file: mod.file + '.html',
                label: `Module ${mod.num}: ${mod.title}`,
                num: mod.num,
                title: mod.title,
                type: 'module',
                id: mod.id
        ...
        // Insert projects after this module
        if (projectsByAfterModule[mod.id]) {
            projectsByAfterModule[mod.id].forEach(p => {
                sidebarPages.push({
                    file: p.file + '.html',
    ...
    function buildModulePage(mod, htmlContent, currentFile, opts) {
        opts = opts || {};
        const sectionTag = opts.sectionTag || ('MODULE ' + mod.num);
        const breadcrumb = opts.breadcrumb || (
            '<a href="index.html">' + courseNameShort + '</a>' +
            '<span class="breadcrumb-sep">/</span>' +
            '<span>Module ' + mod.num + '</span>'
        );
        const pageTitle = opts.pageTitle || ('Module ' + mod.num + ': ' + mod.title);
        const desc = opts.description !== undefined ? opts.description : mod.description;
        const title = opts.title || mod.title;
        const moduleId = opts.moduleId !== undefined ? opts.moduleId : mod.id;
    ...
    modules.forEach(mod => {
        ...
        if (mod.isSplit) {
            const sections = splitModuleSections[mod.id];
            sections.forEach(section => {
                const breadcrumb =
                    '<a href="index.html">' + courseNameShort + '</a>' +
                    '<span class="breadcrumb-sep">/</span>' +
                    '<a href="module' + mod.id + '-1.html">Module ' + mod.num + ': ' + mod.title + '</a>' +
                    '<span class="breadcrumb-sep">/</span>' +
                    '<span>' + section.sectionLabel + '</span>';

                const page = buildModulePage(mod, section.html, section.file, {
                    sectionTag: 'SECTION ' + section.sectionLabel,
                    breadcrumb: breadcrumb,
                    pageTitle: section.sectionLabel + ' ' + section.title + ' | ' + mod.title,
                    title: section.title,
                    description: '',
                    moduleId: mod.id
                });

                fs.writeFileSync(path.join(COURSE_DIST, section.file), page);
                console.log(`  ${section.file} — ${section.title}`);
            });

            // Auto-generate exercises page
            if (mod.hasExercises) {
                const exFile = `module${mod.id}-exercises.html`;
                const exercisesHtml = `
<h3>Warmups</h3>
...
<div id="challenges-container">
    <noscript><p class="js-required">JavaScript is required for the interactive exercises.</p></noscript>
                ...
                const page = buildModulePage(mod, exercisesHtml, exFile, {
                    sectionTag: 'EXERCISES',
                    breadcrumb: breadcrumb,
                    pageTitle: 'Exercises | ' + mod.title,
                    title: mod.title + ' Exercises',
                    description: 'Practice exercises for ' + mod.title,
                    moduleId: mod.id
                });

                fs.writeFileSync(path.join(COURSE_DIST, exFile), page);
                console.log(`  ${exFile}`);
            }
        } else if (fs.existsSync(mdFile)) {
            ...
            const page = buildModulePage(mod, htmlContent, currentFile);
    ...
    // Add active plugin pages to sidebar (after all modules are processed)
    ...
    mkdirp(path.join(COURSE_DIST, 'data'));
    fs.writeFileSync(
        path.join(COURSE_DIST, 'data', 'predict-index.js'),
...
// Network-first strategy: deploys show up on the next reload; the cache
// only serves when the network is unavailable (offline support).
...
// Fetch: network-first for same-origin — fresh responses refresh the
// cache; fall back to cache only when the network fails (offline).
